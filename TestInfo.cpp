...
int main(){
    TestManager manager;
    manager.createTest("TEA000", "Test 123", 10, "1234567", 30, "12:00:00 01/01/2024", "13:00:00 01/01/2024");
    for (int i = 0; i < manager.getTestCount(); i++){
        cout << manager.getTestAt(i).getId() << "/" << manager.getTestAt(i).getTeacherId() << "/" << manager.getTestAt(i).getTitle() << "/" 
        << manager.getTestAt(i).getTotalQuestion() << "/" << manager.getTestAt(i).getPassword() << "/" << manager.getTestAt(i).getDuration() << "/"
        << manager.getTestAt(i).getStartsAt() << "/" << manager.getTestAt(i).getEndsAt() << "/" << manager.getTestAt(i).getStatus() << endl;
    }
    return 0;
}