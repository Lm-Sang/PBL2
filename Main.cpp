...
void ChinhSuaThongTin(int currentSelection, CurrentUser user)
{
    ...
    do
    {
        ...
        else if (Ten)
        {
            ...
            {
                ch = tm.update(id, password, newName);
            }
            else
            {
                ch = sm.update(id, password, newName);
            }
            if (ch) {
            ...
            do
            {
                ...
                else if (key == 13)
                {
                    ...
                    for (int i = 0; i < chapterList.getSize(); i++)
                    {
                        ...
                        {
                            if (qb.getQuestionAt(j).getChapterId() == chapterList[i].getId() && qb.getQuestionAt(j).getSubjectId() == subjectList[k].getId() && qb.getQuestionAt(j).getTeacherId() == user.getId())
                            {
